"""
NSE Stock Alert Bot with Telegram Commands (Railway-friendly, polling-safe)
...
Key fixes:
- Prevents duplicate replies by persisting last_update_id in state.json
- Drops pending Telegram updates ONLY on first boot (when state.json doesn't exist)
- /add sends EXACTLY ONE response message (never a separate warning)
- Alerts trigger ONCE and then become inactive; /update re-activates
"""
...
class TelegramCommandBot:
    def __init__(self, telegram_bot_token: str):
        ...
        # Persist update offset to avoid duplicate processing on restarts
        self.state_file = "state.json"
        self.last_update_id = 0
        self._load_state()

        # Drop backlog ONLY on first boot (no state file)
        if not os.path.exists(self.state_file):
            self._drop_pending_updates()
            # Save immediately so we don't drop again
            self._save_state()
        ...
        try:
            if os.path.exists(self.state_file):
                with open(self.state_file, "r", encoding="utf-8") as f:
                    s = json.load(f)
                    self.last_update_id = int(s.get("last_update_id", 0))
    ...
    def _drop_pending_updates(self):
        """
        Clears any queued updates so we don't process old messages after deploy.
        Only called when state.json does not exist (first boot).
        """
        try:
            url = f"https://api.telegram.org/bot{self.bot_token}/getUpdates"
            # offset=-1 tells Telegram "give me the latest and discard earlier"
            requests.get(url, params={"offset": -1}, timeout=20)
            print("✅ Dropped pending Telegram updates (first boot).")
        except Exception as e:
            print(f"Drop pending updates error: {e}")
    ...
    # -------------------- Commands (ONE reply per command) --------------------
    ...
    def handle_command(self, chat_id: str, message_text: str) -> None:
        ...
        if cmd in ("/start", "/help"):
            self.send_message(
                ...
                (
                    ...
                    "• /add works even if price feed is temporarily down"
                ),
            )
            return
        ...
        if cmd == "/add":
            ...
            try:
                ...
                price = self.get_current_price(f"{stock}.NS")
                ...
                if price is None:
                    msg = (
                        ...
                        f"Lower Limit: ₹{lower}\n"
                        f"Upper Limit: ₹{upper}\n\n"
                        f"⚠️ Price feed temporarily unavailable. Bot will keep checking."
                    )
                else:
                    msg = (
                        f"✅ <b>Alert Added!</b>\n\n"
        ...
        if cmd == "/list":
            ...
            for stock, a in user_alerts.items():
                price = self.get_current_price(a["ticker"])
                status = "✅ Active" if a.get("active") else "⏸️ Inactive (triggered)"
                out += (
                    f"<b>{stock}</b>\n"
                    f"Status: {status}\n"
                    f"Current: {'₹' + str(price) if price is not None else 'N/A'}\n"
        ...
        if cmd == "/remove":
            ...
            self.send_message(chat_id, f"✅ Alert removed for {stock}" if removed else f"❌ No alert found for {stock}")
            return
...
def start_web_server():
    """Tiny HTTP server so Railway sees an open port."""
    app = Flask(__name__)
...
def main():
    ...
    print("\n✅ Bot initialized!")
    print("Send /start or /help to your bot.\n")
